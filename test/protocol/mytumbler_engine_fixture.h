...
struct mytumbler_engine_fixture {
    ...
    static std::string sha256(const std::string& input) {
        ...
        // directly return a string containing 32 bytes of raw data
    ...
    static bool MockAggregateVerifyFunc(const std::string& aggr_sig,
        ...
        for (const auto i : node_set) {
            if (m.count(i) == 0) {
                ...
                std::cout << (int)i << " sign " << toHex(data) << " is " << toHex(MockSignFunc(i, data)) << std::endl;
                std::cout << toHex(m[i]) << " not " << toHex(MockSignFunc(i, data)) << std::endl;
                return false;
            }
        }
        return true;
    ...
    static MyTumblerConfig GenerateMyTumblerConfig(size_t batch_size,
        ...
        cfg.pace_keeping_interval_ = 100;
    ...
    std::shared_ptr<IdToPublicKeyMap> CreatePeers(size_t n = 4, size_t node_index = 0) {
