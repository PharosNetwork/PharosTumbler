...
struct myba_state_machine_fixture {
  ...
  public:
    std::vector<BvalMessagePtr> CreateBvalMessageByBtach(const Seq seq,
                                                         const std::vector<uint32_t> node_index,
                                                         Round round,
        ...
        std::vector<BvalMessagePtr> ret;
        for (size_t i = 0; i < node_index.size(); ++i) {
            auto msg = CreateBvalMessage(seq,
                                         peers_[proposer_index],
                                         round,
                                         hash,
                                         val_exist,
                                         epoch_number);
            std::string tmp_sig =
                MockAggregateSign(node_index[i], CalculateBvalDigest(msg, spec_version));
            msg->signature = std::move(tmp_sig);
            ret.push_back(msg);
        }
        return ret;
    }

    std::vector<AuxMessagePtr> CreateAuxMessageByBatch(const Seq seq,
                                                       const std::vector<uint32_t> node_index,
                                                       const Round round,
                                                       const uint32_t proposer_index,
                                                       const Digest& hash,
                                                       const uint64_t epoch_number,
                                                       const uint16_t spec_version) {
        std::vector<AuxMessagePtr> ret;
        auto bval_digest = CalculateBvalDigest(seq,
                                               peers_[proposer_index],
                                               round,
                                               hash,
                                               epoch_number,
        ...
        for (size_t j = 2; j <= 5; ++j) {
            MockAggregateSignature(tmp_bval_sig, MockAggregateSign(j, bval_digest));
        }
        std::set<NodeId> tmp_ids{peers_[1], peers_[2], peers_[3], peers_[4], peers_[5]};
        std::string bitmap;
        engine_->crypto_helper_->GenerateBitMap(tmp_ids, bitmap);
        for (size_t i = 0; i < node_index.size(); ++i) {
            ssz_types::AggregateSignature aux_proof;
            aux_proof.bitmap = bitmap;
            aux_proof.aggr_sig = tmp_bval_sig;
            auto msg =
                CreateAuxMessage(seq, peers_[proposer_index], round, hash, std::move(aux_proof));
            Signature tmp_sig = MockAggregateSign(node_index[i], CalculateAuxDigest(msg));
            msg->signature = std::move(tmp_sig);
            ret.push_back(msg);
        }
        return ret;
    ...
    std::vector<PromMessagePtr> CreatePromMessageByBatch(const Seq seq,
                                                         const std::vector<uint32_t> node_index,
                                                         const Round round,
                                                         const uint32_t proposer_index,
                                                         const Digest& hash,
        ...
        for (size_t i = 0; i < node_index.size(); ++i) {
            ...
            prom_proof.bitmap = bitmap;
            prom_proof.aggr_sig = tmp_bval_sig;
            auto msg =
                CreatePromMessage(seq, peers_[proposer_index], round, hash, std::move(prom_proof));
            Signature tmp_sig = MockAggregateSign(node_index[i], CalculatePromDigest(msg));
            msg->signature = std::move(tmp_sig);
            ret.push_back(msg);
        }
        return ret;
